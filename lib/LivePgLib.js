...
// var TRIGGER_QUERY_TPL = await Assets.getTextAsync('lib/trigger.tpl.sql');
// var REFRESH_QUERY_TPL = await Assets.getTextAsync('lib/refresh.tpl.sql');
...
const LivePgLib = function(settings, channel) {
  ...
  initialConnect(async function(error, client, done){
...
LivePgLib.prototype._initTriggerFun = async function() {
  var self = this;
  let TRIGGER_QUERY_TPL = await Assets.getTextAsync('lib/trigger.tpl.sql');
  self.query(
...
LivePgLib.prototype._updateQuery = function(queryHash, callback) {
  ...
  pg.connect(self.connStr, async function(error, client, done) {
    if(error) return callback && callback(error);
    let REFRESH_QUERY_TPL = await Assets.getTextAsync('lib/refresh.tpl.sql');
    client.query(
